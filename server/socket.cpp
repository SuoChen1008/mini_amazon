...
/*
    Work as a client, create a socket to request connection to server, based on hostName and portNum.
...
int clientRequestConnection(const string & hostName, const string & portNum) {
  ...
  status =
      connect(client_fd, host_info_list->ai_addr, host_info_list->ai_addrlen);  //建立连接
  if (status == -1) {
